 ...
 * @author 52811
 */
@WebServlet(name = "ShowNewController", urlPatterns = {"/ShowNewController"})
public class ShowNewController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        news noticia = newsDAO.getNewsID(idNew);
        String nameUser = (String)session.getAttribute("name");
        if(newsDAO.isNewMarkedbyUser(nameUser, idNew)){
            session.setAttribute("isMarked", "1");
        }else{
            session.setAttribute("isMarked", "0");
        }
